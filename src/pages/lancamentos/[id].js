...
export default function Lancamentos(props) {
  ...
  const updateForm = async () => {
    await get(child(ref(database), `/${db_name}/projetos/${paramId}/despesas`)) //`/${db_name}/projetos/${paramId}/despesas`))
      .then((snapshot) => {
        setDados([]);
        if (snapshot.exists()) {
  ...
  const handleDelDespesas = async (id) => {
    const dbRef = ref(
      database,
      `/${db_name}/projetos/${paramId}/despesas/` + id
    );
    ...
    if (descric !== "" && fornecedor !== "" && data !== "" && valor !== "") {
      ...
      async function salvarDados(downloadURL) {
        ...
        if (editMode === false) {
          ...
          const updates = {};
          updates[`/${db_name}/projetos/${paramId}/despesas/` + idLancamento] =
            lancamento;
        ...
        //SE ESTIVER EM MODO DE EDIÇÃO UTILIZA O ID QUE ESTÁ NO STATE "ID"
          ...
          const updates = {};
          updates[`/${db_name}/projetos/${paramId}/despesas/` + id] =
            lancamento;
          await update(ref(database), updates)
            .then(() => {
              updateForm();
